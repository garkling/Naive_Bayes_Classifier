...
tidy_text <- unnest_tokens(train, 'word', 'tweet', token="words") %>%
...
word_counts <- tidy_text %>% count(label, word)
...
# count the total number of words for each label
totals <- tidy_text %>%
  count(label)

totals <- setNames(totals$n, totals$label)

unique_words <- distinct(tidy_text, word)
word_probabilities <<- word_counts %>% mutate(prob = (n + 1) / (totals[label]))

tweet_counts$probability[tweet_counts$label == "discrims"]
...
naiveBayes <- setRefClass("naiveBayes",
       ...
       methods = list(
                    ...
                    {
                         words <- unlist(strsplit(tolower(message), "\\s+"))
                         ...
                         for(class in classes) {
                           ...
                           for (word in words) {
                             if (word %in% vocabulary$word) {
                               ...
                               word_probability <- 1 / (total_word_counts[class] + vocabulary_size)
                             }
